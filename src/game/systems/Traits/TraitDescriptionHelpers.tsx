...
export function buildTierGroups(trait: Trait) {
    ...
    const stageParams = ordered.map(([, v]) => (v as any)?.descriptionParams as (string | number)[] | undefined).filter(Boolean) as (
        | string
        | number
    )[][]
    ...
    for (let paramIdx = 0; paramIdx < maxLen; paramIdx++) {
        const col = stageParams
            .map((arr) => arr[paramIdx])
            .filter((v) => v != null)
            .map(String)
        ...
        groups.push(
            <Typography key={`grp-${paramIdx}`} component="span" sx={{ fontWeight: 700, fontSize: "inherit" }}>
                {"("}
                {col.map((val, i) => (
