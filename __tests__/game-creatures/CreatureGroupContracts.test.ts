...
const boardLimitPath = join(root, "src/game/creature/character/boardLimit.ts")
...
describe("PlayerTeam contracts", () => {
    ...
    it("saves, augments, benches, and recalculates traits through EventBus", () => {
        const source = readSource(playerTeamPath)
        const boardLimitSource = readSource(boardLimitPath)
        ...
        expect(source).toContain("getMaxBoardCharacters()")
        expect(source).toContain("calculateMaxCharactersInBoard(this.getHighestLevelChar())")
        expect(boardLimitSource).toContain("calculateMaxCharactersInBoard")
...
describe("Character contracts", () => {
    it("extends Creature with level badge, board coordinates, DTO loading, and trait reapplication", () => {
        ...
        expect(source).toContain("const savedItems = dto.items.includes(\"thiefsgloves\") ? [\"thiefsgloves\"] : dto.items")
    ...
    it("serializes, levels, updates UI, and persists item equip changes", () => {
        ...
        expect(source).toContain(".filter((item) => !item.temporarySource)")
        expect(source).toContain("override equipItem")
        expect(source).toContain("this.scene.availableItems.delete(item)")
        expect(source).toContain("!this.scene.isHydratingSavedRun && !fromThiefsGloves")
        expect(source).toContain("this.team.saveCurrentCharacters()")
        expect(source).toContain("this.scene.saveProgress()")
