...
Durante o combate, Rokmora navega em ciclo pelas constelações `archer`, `dragon` e `chalice`, cada uma com duração própria definida na regra. A constelação do Arqueiro dispara uma flecha luminosa azul-violeta a partir do alvo atacado contra um inimigo próximo, causando dano radiante. A constelação do Dragão concede escudo ao receber dano, calculado a partir do dano efetivamente recebido e da armadura atual de Rokmora. A constelação da Taça pulsa periodicamente uma aura verde, distribuindo cura entre todos os aliados feridos, incluindo ele próprio.
