...
Sua passiva divide o slime quando ele cruza o limite de vida definido pela regra da criatura sem morrer. A vida atual vira a nova vida máxima das duas metades, ambas reduzem o tamanho visual, e o slime deixa de dividir ao atingir o tamanho mínimo. A divisão chama `removeFromEnemyTarget` para permitir que os inimigos redistribuam alvo entre as metades. A nova metade sempre nasce como o slime azul original e recebe cópias independentes dos itens do original.

Sua ativa faz o slime crescer, curar a si mesmo com base em AP e então acionar a divisão imediatamente. O cast não cria proxies ou transformação em outra unidade; ele reutiliza a regra de divisão existente, de modo que o tamanho aumentado influencia diretamente o tamanho das metades resultantes. Durante o splash da divisão, tanto o slime original quanto o clone ficam temporariamente invulneráveis, sem atacar, sem mover e não-alvejáveis. Clones criados pela divisão são marcados como temporários e removidos pelo `EnemyTeam` no reset do floor, antes do reposicionamento dos monstros do próximo encontro.
